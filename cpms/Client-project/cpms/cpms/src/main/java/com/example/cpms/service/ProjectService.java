...
public class ProjectService {
    ...
    //  Return ProjectEntity (let controller convert to DTO)
    ...
    //  Return List<ProjectEntity> (let controller convert to DTOs)
    ...
    // Return ProjectEntity (let controller convert to DTO)
    public ProjectEntity updateProject(Long id, ProjectRequest request, UserEntity user) {
        // 1. Find the existing project
        ProjectEntity project = projectRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Project not found with id: " + id));
