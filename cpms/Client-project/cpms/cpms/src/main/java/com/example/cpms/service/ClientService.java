...
//                return ApiResponse.error("Client with email " + request.getEmail() + " already exists");
//            }
//
//            //Check for duplicate name for the same user
//            if (clientRepository.findByUserAndName(user, request.getName()).isPresent()) {
//                return ApiResponse.error("Client with name '" + request.getName() + "' already exists for you");
//            }
//
//
...
public class ClientService {
    ...
    //  Return List<ClientEntity> (let controller convert to DTOs)
    ...
    //  Return ClientEntity (let controller convert to DTO)
    public ClientEntity updateClient(Long id, ClientRequest request, UserEntity user) {
        ClientEntity client = clientRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Client not found with id: " + id));
    ...
    public void deleteClient(Long id, UserEntity user) {
        ClientEntity client = clientRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Client not found with id: " + id));
        ...
        clientRepository.delete(client);
        //  Let controller handle success response
    }
}